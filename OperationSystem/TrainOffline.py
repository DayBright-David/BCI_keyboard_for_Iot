...
# 创建实验信息从脚本函数中生成
personName = input('Enter subject name:')
paradigm = input('Enter paradigm type:')

config = Config()
config.subINFO(personName=personName)
config.expINFO(winLEN=0.5, trainBlockNUM=8, testBlockNUM=0, paradigm=paradigm,chnNUM=9)
config.connectINFO(streaming_ip='10.0.0.3',streaming_port=4000,host_ip='10.0.0.2')
