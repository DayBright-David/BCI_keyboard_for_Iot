...
class NeuroScanEEGThread(threading.Thread,):
    def __init__(self, config=None,hostname='0.0.0.0', port=4000,threadName='NeuroScanEEG', device='NeuroScan', n_chan=2, record_srate=1000,srate=250,keepEvent=False):
        ...
        # cash 数据和event缓存
        self.event = None
        self.eventPoint = deque(maxlen=1500)
        ...
        self.messageQueue = deque(maxlen=36000)  # 缓冲的数据队列
        self.track = deque(maxlen=36000)
    ...
    def readFixedData(self, startPoint, length):
        ...
        while len(self.eventPoint) != 0:
            ...
            return data if self.keepEvent == False else data,eventType
        else:
            return None
    ...
    def _addEvent(self):
        ...
        # print('Received [{s}] kBytes of EEG Data,start at [{t}]'.format(
        #     s=str(len(packet)/1000), t=startData))
